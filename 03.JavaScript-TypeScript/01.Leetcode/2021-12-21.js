 ...
 * @returns
 */
function minWindow(longStr, shortStr) {
  // 采用滑动窗口法，双指针
  ...
  var mapExpand = function (_pMap, key) {
    if (!_pMap.has(key)) {
      _pMap.set(key, 1);
    } else {
  ...
  var containNeeds = function (_pMap) {
    // 构建所需子串Map的键的可遍历数组
    var needKeys = needsMap.keys();
    for (var _i = 0, needKeys_1 = needKeys; _i < needKeys_1.length; _i++) {
      var needKey = needKeys_1[_i];
...
var result = minWindow('ADOBECODEBANC', 'ABC');
